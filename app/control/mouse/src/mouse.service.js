(function () {
    'use strict';

    angular
        .module('app')
        .service('mouseService', mouseService);
    //TODO убрать Service в названии

    mouseService.$inject = ['$log'];

    function mouseService($log) {
        var mouseHandlers = {};

        this.emitMouseEvent = emitMouseEvent;
        this.addMouseHandler = addMouseHandler;

        function emitMouseEvent(event, evtObj, cellX, cellY) {
            if (!mouseHandlers[event]) {
                $log.error('mouseEvent: ' + event + ' - no handlers');
                return;
            }
            mouseHandlers[event].forEach(function (handleItem, index) {
                handleItem(evtObj, +cellX, +cellY, function (result) {
                    $log.debug('handling ' + index + ' ' + event + ' finished - result: ' + result);
                });
            });
        }

        function addMouseHandler(event, handler) {
            if (!mouseHandlers[event]) {
                mouseHandlers[event] = [];
            }
            mouseHandlers[event].push(handler);
        }
    }

})();
